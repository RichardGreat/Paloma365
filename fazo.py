...
class Column:
    ...
    def make_type_nvl(self, value: str):
        if "text" == self.column_type:
            return f"nvl({value}, null)"
        elif "int" == self.column_type or "integer" == self.column_type:
            return f"nvlTryInt({value})"
        elif "real" == self.column_type:
            return f"nvlTryNum({value})"
        else:
            return f"nvl({value}, null)"
...
class GenerateZ:
    ...
    def __init(self):
        ...
        nullable = "?"
        columns = [
            f"{req_ann if c.is_required else emp_str}{c.get_type_object()}{emp_str if c.is_required else nullable} {c.get_name2class()}"
            for c in t.columns]
        b.append("\t// init")
    ...
    def __update_by_map(self):
        ...
        where_pk = ["${" + t.get_name2class() + "." + x.get_name2constant() + "} = ?" for x in
                    pk_columns]
        where_pk = " AND ".join(where_pk)
        where_args = [f"data[{t.get_name2class()}.{x.get_name2constant()}]" for x in pk_columns]
        where_args = ", ".join(where_args)
    ...
    def __update_one(self):
        ...
        emp_str = ""
        method_params = [
            f"{req_ann if c.name in pk_names else emp_str}{c.get_type_object()}{emp_str if c.name in pk_names else nullable} {c.get_name2class()}"
            for c in t.columns]
        b.append("\t// update by one")
        b.append(
            "\tstatic Future<int> updateOne(dynamic db, {" + ", ".join(
                method_params) + ", bool removeNull = false}) {")
        args = ", ".join([x.get_name2class() + ": " + x.get_name2class() for x in t.columns])
        b.append(
            "\t\treturn updateByMap(db, toMap(" + args + "), removeNull: removeNull);")
        b.append("\t}")
    ...
    def __save_row_by_map(self):
        ...
        b.append("\t\t\tdata.removeWhere((key, value) => value == null);")
        b.append("\t\t}")
        b.append("\t\t" + t.get_name2class() + ".checkRequiredByMap(data);")
        b.append("\t\tif (db is Batch) {")
        b.append("\t\t\tdb.insert(" + t.get_name2class() +
                 ".TABLE_NAME, data, conflictAlgorithm: ConflictAlgorithm.replace);")
        b.append("\t\t\treturn Future.value(-1);")
    ...
    def __to_row_from_map(self):
        ...
        args = ", ".join([
            f"{c.get_name2class()}: {c.get_name2class()}{force_wrap if c.is_required else empty_str}"
            for c in t.columns])
        b.append(f"\t\treturn new {t.get_name2class()}({args});")
        b.append("\t}")
    ...
    def __to_map_from_list_string(self):
        b = self.buffer
        t = self.table

        b.append("\t// to map from list strings")
        b.append(
            "\tstatic Map<String, dynamic> toMapFromListString({required List<String> values}) {")
        values = [
            f"\t\t\t{t.get_name2class()}.{c.get_name2constant()}: values.length > {i} ? {c.make_type_nvl(f'values[{i}]')} : null"
            for i, c in enumerate(t.columns)]
    ...
    def run(self):
        ...
        b.append("")
        self.__to_map_from_list_string()
        ...
        b.append("")
        b.append("\tstatic int? nvlTryInt(Object value) {")
        b.append("\t\tif (value is String)")
        b.append("\t\t\treturn value.isNotEmpty ? int.tryParse(value) : null;")
        b.append("\t\telse if (value is int)")
        ...
        b.append("")
        b.append("\tstatic num? nvlTryNum(Object value) {")
        b.append("\t\tif (value is String)")
        b.append("\t\t\treturn value.isNotEmpty ? num.tryParse(value) : null;")
        b.append("\t\telse if (value is num)")
        b.append("\t\t\treturn value;")
        b.append("\t\telse if (value is int)")
        b.append("\t\t\treturn value;")
        b.append("\t\telse if (value is double)")
        b.append("\t\t\treturn value;")
        b.append("\t\telse")
        b.append("\t\t\treturn null;")
        b.append("\t}")
...
def generate_table_class(buffer: list, table: Table):
    ...
    method_params = [f"{x.get_name2constant()}" for x in table.columns if x.is_required]
    if len(method_params) > 0:
        # check requires by map
        buffer.append("")
        buffer.append("\tstatic void checkRequiredByMap(Map<String, dynamic> data) {")
        checks = [f"\t\tArgumentError.checkNotNull(data[{x}], {x});"
    ...
    for c in table.columns:
        buffer.append(
            f"\tfinal {c.get_type_object()}{empty_str if c.is_required else nullable} {c.get_name2class()};")
    ...
    buffer.append("")
    buffer.append(
        f"\t{table.get_name2class()}({constructor_params});")
...
def run(path):
    ...
    if len([x for x in files if x.endswith(".sqlite")]) > 0:
        for file_name in [x for x in files if not x.endswith(".sqlite")]:
